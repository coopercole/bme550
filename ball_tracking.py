...
def track_ball(video, tracked_points, mask_lower, mask_upper, show_video=SHOW_VIDEO):
    ...
    out = cv2.VideoWriter('output2_30fps.mp4', cv2.VideoWriter_fourcc(*'mp4v'), fps, (width, height))
    while True:
...
def plot_scatter(x1, y1, x2, y2, title1, title2):
    if x2 is None or y2 is None:
        plt.figure()
        plt.scatter(x1, y1, label=title1)
        plt.gca().invert_yaxis()
        plt.xlabel('x-coordinate (pixels)')
        plt.ylabel('y-coordinate (pixels)')
        plt.legend()
        ...
        plt.figure()
        plt.scatter(x1, y1, label=title1)
        plt.scatter(x2, y2, label=title2)
        plt.gca().invert_yaxis()
        plt.xlabel('x-coordinate (pixels)')
        plt.ylabel('y-coordinate (pixels)')
        plt.legend()
...
if not ITERATE_THROUGH_VIDEOS:
    # video = 'high_n.MOV'
    # outgoing traj under incoming:
    # video = 'project_videos\\high\\Pocket\\20231116_211230000_iOS.MOV'
    # incoming traj under outgoing:
    video = 'project_videos\\high\\Pocket\\20231116_211328000_iOS.MOV'
    ...
    if SHOW_PLOTS:
        ...
        # plot_scatter(x_coords, y_coords, None, None, 'Ball trajectory', None)
        # plot the incoming and outoging trajectories
        plot_scatter(incoming_x, incoming_y, outgoing_x, outgoing_y, 'Incoming trajectory', 'Outgoing trajectory')
    ...
    for video_type_name, video_type in video_files.items():
        for shot_type, videos in video_type.items():
            for video in videos:
                ...
                # Initialize the list of points
                # ball_x_distance = (get_ball_distance_to_edge(impact_frame) / PIXELS_PER_METER * 100, 2)
                # print(f"Distance from ball to edge: {ball_x_distance} centimeters")
